        ...
        if not content_type and original_image_bytes:
            try:
                with Image.open(io.BytesIO(original_image_bytes)) as img:
                    ...
                    if content_type:
                        logger.info(
                            "process_image_url_to_aicarus_seg: 通过Pillow从文件内容猜测到"
                            f"Content-Type: {content_type}。URL: {safe_url}"
                        )
                    else:
                        logger.warning(
                            "process_image_url_to_aicarus_seg: Pillow未能从文件内容猜测到"
                            f"Content-Type ({img_format})。URL: {safe_url}"
                        )
            except Exception as e:
                logger.warning(
                    "process_image_url_to_aicarus_seg: 使用Pillow猜测"
                    f"Content-Type时发生错误: {e}。URL: {safe_url}"
                )

        if not content_type:
            logger.warning(
                "process_image_url_to_aicarus_seg: 最终未能获取到Content-Type，"
                f"将使用默认值 'application/octet-stream'。原始URL: {safe_url}"
            )
            content_type = "application/octet-stream"

        logger.debug(
            "process_image_url_to_aicarus_seg: 准备保存媒体文件，"
            f"最终确定的Content-Type: {content_type}, "
            f"URL: {safe_url}"
        )
    ...
    try:
        async with (
            ...
            if response.status == 200:
                ...
                logger.debug(
                    "get_content_type_from_url: URL: {url}, "
                    f"HTTP Status: {response.status}, "
                    f"Raw Content-Type Header: {raw_content_type}"
                )
                if raw_content_type:
                    # 清理掉可能存在的 charset 等附加信息
                    cleaned_content_type = raw_content_type.split(";")[0].strip()
                    logger.debug(
                        f"get_content_type_from_url: URL: {url}, "
                        f"Cleaned Content-Type: {cleaned_content_type}"
                    )
                    return cleaned_content_type
                else:
                    logger.warning(
                        "get_content_type_from_url: 获取 Content-Type 失败 "
                        f"(HTTP {response.status}): {url}，响应头中未找到 Content-Type。"
                        "将返回 None。"
                    )
                    return None
            else:
